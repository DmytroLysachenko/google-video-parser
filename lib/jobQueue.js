...
async function acquireJobSlotWithMemoryGuard() {
  ...
  let memoryWarningSent = false;
  let slotWarningSent = false;
  ...
  while (true) {
    ...
    if (rssMb > MAX_MEMORY_MB) {
      if (!memoryWarningSent) {
        console.warn(
          `Memory ${rssMb.toFixed(
            1
          )}MB exceeds limit ${MAX_MEMORY_MB}MB. Waiting before next job...`
        );
        memoryWarningSent = true;
      }
      slotWarningSent = false;
      ...
      if (!slotWarningSent) {
        ...
        slotWarningSent = true;
      }
      memoryWarningSent = false;
    }
